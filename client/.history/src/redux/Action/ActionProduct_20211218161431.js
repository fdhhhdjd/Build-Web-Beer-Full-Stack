...
export const getAdminCategoryStart = () => ({
  type: types.GET_ADMIN_CATEGORY_START,
});
export const getAdminCategorySuccess = (api) => ({
  type: types.GET_ADMIN_CATEGORY_SUCCESS,
  payload: api,
});
export const getAdminCategoryFail = (error) => ({
  type: types.GET_ADMIN_CATEGORY_FAIL,
  payload: error,
});
...
export const GetAdminProductInitialState = (
  pageAdmin,
  category,
  sort,
  search
...
export const GetCategoryInitialState = (token) => {
  return async function (dispatch) {
    dispatch(getAdminCategoryStart());
    await axios
      .get("/api/category")
      .then((item) => {
        dispatch(getAdminCategorySuccess(item.data));
      })
      .catch((error) => {
        dispatch(getAdminCategoryFail(error));
      });
  };
};
